...
//    * @return {[type]}     [description]
//    */
//   getPropertyValueDom(propertyValue, elasticClassName, formatValue) {
...
//    * @return {[type]}                dom字符串
//    */
//   mapKey(paramParentObj, paramKeywords, paramParentName) {
//     var _this = this,
//       tempKeywords = '<b class="redword">{#name}</b>',
...
//     }
//   },
//   mapValue(paramParentObj, paramKeywords, paramParentName) {
//     var _this = this,
//       loopResultStr = _this.$resultList[0],
//       isDeep = _this.$labCheckedTree.hasClass('del') ? false : true,
//       cacheValue = null,
...
//   buildSearchResultDom(showKey, showValue, searchBoxValue, loopResultStr) {
//     var _this = this;
...
//     this.setState({jsonDataStr: e.currentTarget.value});
//   }

//   render() {
//     const {jsonDataStr} = this.state;
//     return (
//       <section className={classnames(styles.content, styles["object-scan"])}>
//         <div className={styles["left-area"]}>
//           <textarea
//             value={jsonDataStr}
//             onChange={this.onJsonDataChange}
//             placeholder="请输入合法的JSON，按tab键（失焦自动格式化），定位到搜索框，输入合法的属性名进行属性查找，Aa,Tree,Value是功能键；Aa: 如果输入的是字母，区分大小写，默认不区分；Tree: 深度查找JSON对象,默认深度查找；Value: 可按照属性值来进行查找，默认按照属性名；"
//             className={classnames(styles["area-text"], styles.txtObjectString)} tabIndex={-1}></textarea>
//         </div>
//         <div className={styles["obj-scan"]}>
//           <div className={styles["parse-result"]}>
//             <span className={classnames(styles["rate-of-progress"], styles.progressRate)} style={{display: 'none'}}>解析中...
//               <i></i>
//             </span>
//             <span className={classnames(styles["progress-success"], styles.progressSucc)} style={{display: 'none'}}>解析成功</span>
//             <span className={styles["msg-error msg_str"]} style={{display: 'none'}}>字符串不合法</span>
//           </div>
//           <div className={styles["txtSearchArea"]}>
//             <input type="search" placeholder="属性名/属性值" value="" className={classnames(styles.txt, styles["txt-50"], styles.searchTxt)} />
//             <ul className={classnames(styles["txt-result-list"], styles.txtResultList)} tabIndex={1} style={{display: 'none'}}></ul>
//             <label className={classnames(styles["lab-tip"], styles["lab-checked-Aa"])} title="是否区分大小写，默认否">Aa</label>
//             <label className={classnames(styles["lab-tip"], styles["lab-checked-Tree"])} title="是否进行深度遍历，默认是">Tree</label>
//             <label className={classnames(styles["lab-tip"], styles["lab-checked-Value"])} title="是否进行按值(boolean、string、number、null)搜索，默认否">Value</label>
//           </div>
//           <div className={styles["printObjTree"]}>
//             <div className={classnames(styles["print-property-name"], styles.printPropertyName)}>&gt;</div>
//             <textarea readOnly className={classnames(styles["print-property-value"], styles.printPropertyValue)}></textarea>
//           </div>
//         </div>
//       </section>
//     );

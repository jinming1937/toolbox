...
export const FormatVariable = (props: IFormatVariable) => {
  ...
  const onLineTypeChange = useCallback((e: ChangeEvent<HTMLSelectElement>) => {
  ...
  const onOriginValuePaste = useCallback((e: ClipboardEvent<HTMLInputElement>) => {
    setTimeout(() => {
      const val = originRef.current !== null ? originRef.current.value : '';
      console.log(val, originValue, e.persist());
      let newStr = ''
      if (isCamelToPascal) {
        newStr = val.replace(/^([A-Z])/, (target, wanted) => {
            console.log(wanted);
  ...
  const onOriginValueBlur = useCallback((e: FocusEvent<HTMLInputElement>) => {
    setOriginValue('');
  }, [originValue]);
  ...
  const handleCopy = useCallback((text: string, result: boolean) => {
    setCopyState(result);
  ...
  return (
    <div className={styles.formatVariable} style={style}>
      ...
      <div className={styles.clearBox}>
        <label htmlFor="cbxNeedClear">是否在离开（失焦）时擦除 <input type="checkbox" checked={leaveClear} onChange={onClearControlChange} /></label>
      </div>
      {isCamelToPascal

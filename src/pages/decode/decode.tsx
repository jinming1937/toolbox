...
export const Decode = (props: IDecode) => {
  ...
  const onUrlTextAreaPaste = useCallback((e: React.ClipboardEvent<HTMLTextAreaElement>) => {
    setTimeout(() => {
  ...
  const onUrlTextAreaBlur = useCallback((e: React.FocusEvent<HTMLTextAreaElement>) => {
    if (isBlurClear) setUrlValue('')
  }, [isBlurClear])

  const onClearChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    setIsBlurClear(e.currentTarget.checked);
  }, [isBlurClear])

  const onFormatChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    setIsFormat(e.currentTarget.checked);
  }, [isFormat]);

  const onUrlTextAreaChange = useCallback((e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    <main className={styles['main-box']}>
      <section className={styles['context']}>
        <div className={styles['textarea-face']}>
          <textarea ref={originRef} value={urlValue} onPaste={onUrlTextAreaPaste} onBlur={onUrlTextAreaBlur} onChange={onUrlTextAreaChange} cols={30} rows={10} className={styles['parse-url-area']} placeholder="把想要decode的链接粘贴至此"></textarea>
        </div>
        <div>
          <label className={styles['tool-lab']} htmlFor="isClear_cbx">失焦清空：</label>
          <input className={styles['tool-cbx']} type="checkbox" id="isClear_cbx" checked={isBlurClear} onChange={onClearChange} />
          <label className={styles['tool-lab']} htmlFor="isAutoChangeLine_cbx">参数换行：</label>
